    ...
    <nav className="bg-gray-900 shadow-lg w-full relative z-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            {/* Added onClick to the logo link to also close the menu */}
      ...
      <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:hidden bg-gray-800 border-t border-gray-700`}>
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          {/* Added onClick to all links to close the menu after navigation */}
          <Link to="/" onClick={closeMobileMenu} className="text-gray-300 hover:text-white block px-3 py-2 rounded-md text-base font-medium">Home</Link>
          <Link to="/flights" onClick={closeMobileMenu} className="text-gray-300 hover:text-white block px-3 py-2 rounded-md text-base font-medium">Flights</Link>
          <Link to="/shipping" onClick={closeMobileMenu} className="text-gray-300 hover:text-white block px-3 py-2 rounded-md text-base font-medium">Shipping</Link>
          <Link to="/contact" onClick={closeMobileMenu} className="text-gray-300 hover:text-white block px-3 py-2 rounded-md text-base font-medium">Contact Us</Link>
        </div>
        <div className="border-t border-gray-700 pt-4 pb-3">
          <div className="px-2 space-y-2">
            <Link to="/signup" onClick={closeMobileMenu} className="block text-center font-semibold text-white bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 px-5 py-2 rounded-md transition-all duration-300 shadow-lg">
              Sign Up
            </Link>
            <Link to="/login" onClick={closeMobileMenu} className="block text-center font-semibold text-blue-400 bg-transparent border border-blue-500 px-5 py-2 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300">
              Login
            </Link>
          </div>
        </div>
